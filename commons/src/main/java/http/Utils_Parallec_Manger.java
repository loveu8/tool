...
public enum Utils_Parallec_Manger {
  ...
  public void prepareHttpGet(String filePath) {
    ParallelClient pc = new ParallelClient();
    pc.prepareHttpGet("").setConcurrency(1000)
    ...
    if (pc.isClosed.get()) {
      ...
      for (ResponseOnSingleTask res : responseSontents) {
        logger.info("url = " + res.getHost() + " , " + "statusCode = " + res.getStatusCode() + ", "
            + "statusCodeInt = " + res.getStatusCodeInt() + " , " + "length = "
            + res.getResponseContent().length());
